# -*- encoding: utf-8 -*-
# $Id: exceptions.py,v 1.9 2008/05/30 02:50:21 jwp Exp $
##
...
class Class(State):
	...
	def __repr__(self):
		ordered_states = [
			(k, v._chars[2:]) for k, v in self.__dict__.iteritems()
	...
	def __iter__(self):
		return iter([
			x for x in self.__dict__.itervalues() if isinstance(x, State)
		])
...
def msgstr(ob):
	...
	return str(message or details.get('message')) + (details is not None and
		linesep + linesep.join(
			[
				'%s: %s' %(k.upper(), v) for k, v in details.iteritems()
...
class TransactionError(Error): pass
class IntegrityError(Error): pass
class ValidityError(Error): pass

class PythonError(Error): code = PYTHON
class PythonExceptionError(PythonError):
	"""
	A Python exception was raised by the backend.
	"""
