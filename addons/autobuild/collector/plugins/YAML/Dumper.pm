...
sub is_valid_plain {
    ...
    return 0 if $_[0] =~ /\:(\s|$)/;
    return 0 if $_[0] =~ /[\s\|\>]$/;
    return 0 if $_[0] =~ /^[yY]|[yY]es|YES$/;
    return 0 if $_[0] =~ /^[nN]|[nN]o|NO$/;
    #return 0 if $_[0] =~ /^[tT]|[tT]rue|TRUE$/;
    #return 0 if $_[0] =~ /^[fF]|[fF]alse|FALSE$/;
    return 0 if $_[0] =~ /^[oO]n|ON$/;
    return 0 if $_[0] =~ /^[oO]ff|OFF$/;
    return 1;
}
